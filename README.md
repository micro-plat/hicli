...
```
> **提示**  
> hicli通过配置字段**约束**来生成创建文件，具体可参考[约束配置]();  
> hicli也可以根据数据库，直接创建与之对应的数据字典，命令如下：
```
hicli dic create  -db 数据库连接串 -cover -f
```
> | 数据库类型 | 数据库连接串       |
> | ------ | ---------- | 
> | ORACLE     | oracle:userName/pwd@tnsName |
> | MYSQL     | mysql:userName:pwd@tcp(host:port)/dbName |
...
在web目录下使用命令：
```sh
> hicli ui page ../docs/dic.md src/pages/ -t oms_down_channel -f -cover  
```
生成页面文件：  
![](./img/page_create.bmp)  
...
在webserver目录命令：
```sh
> hicli app service ../docs/dic.md -t oms_down_channel -f -v
```
生成服务层文件：  
